...
newtype HumanReadableRepositoryName = HumanReadableRepositoryName { unHumanReadableRepositoryName :: Text }
  deriving (ToField, FromField, ToJSON, Parsable, Eq, Show)

newtype UrlSafeRepositoryName = UrlSafeRepositoryName { unUrlSafeRepositoryName :: Text }
  deriving (ToField, FromField, ToJSON, Parsable, Eq, Show)

data OutgoingRepositoryData = OutgoingRepositoryData
  { _outgoingRepositoryDataId :: RepositoryId
  , _outgoingRepositoryDataHumanReadableName :: HumanReadableRepositoryName
  , _outgoingRepositoryDataUrlSafeName :: UrlSafeRepositoryName
  }

instance FromRow OutgoingRepositoryData where
  fromRow = OutgoingRepositoryData <$> field <*> field <*> field

instance ToJSON OutgoingRepositoryData where
  toJSON OutgoingRepositoryData{_outgoingRepositoryDataId=repoId, _outgoingRepositoryDataHumanReadableName=humanReadableName, _outgoingRepositoryDataUrlSafeName=urlSafeName} = object
    [ "id" .= repoId
    , "human-readable-name" .= humanReadableName
    , "url-safe-name" .= urlSafeName
    ]
...
data DashboardData = DashboardData
  ...
  , _dashboardDataOutgoingRepoData :: [OutgoingRepositoryData]
  }
...
allRepoIdsForUserIdQuery :: Connection -> UserId -> IO [OutgoingRepositoryData]
allRepoIdsForUserIdQuery conn userId =
  query conn "SELECT id, human_readable_name, url_safe_name FROM repositories WHERE owner_user_id = ?" (Only userId)

outgoingRepoDataForUserId :: UserId -> IO [OutgoingRepositoryData]
outgoingRepoDataForUserId userId =
...
getTopLevelDashboard userId =
  do
    ...
    outgoingRepositoryData <- outgoingRepoDataForUserId userId
    pure (DashboardData{_dashboardDataPackages=packages, _dashboardDataAuthTokens=authTokens, _dashboardDataOutgoingRepoData=outgoingRepositoryData})
